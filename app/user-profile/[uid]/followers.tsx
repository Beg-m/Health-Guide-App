...
export default function FollowersScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const snap = await getDocs(collection(db, "users", profileUid, "followers"));
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.background }} edges={["top"]}>
      <View style={s.header}>
        ...
        <Text style={s.title}>Takipçiler</Text>
        <View style={{ width: 24 }} />
      </View>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={s.empty}>Henüz takipçi yok</Text>}
        />
      )}
    </SafeAreaView>
  );
